--create database lesson_18_hw_18
--use lesson_18_hw_18
...
-- Step 2: Insert aggregated sales data for the current month
...
-- Step 3: Query the results (optional)
...
DECLARE @N INT = 3;  -- Example: 3rd highest

SELECT
    (SELECT DISTINCT Salary
     FROM Employee
     ORDER BY Salary DESC
     OFFSET @N - 1 ROWS FETCH NEXT 1 ROWS ONLY) AS NthHighestSalary;
...
+----+-----+
| 3  | 3   |
Explanation: The person with id 3 is a friend of people 1, 2, and 4, 
...
WITH Numbers AS (
    ...
    FROM master.dbo.spt_values  -- or use a Numbers table if available
)
SELECT n.id
FROM Numbers n
LEFT JOIN MyTable m ON n.id = m.id
WHERE n.id BETWEEN 1 AND (SELECT MAX(id) FROM MyTable)
  AND m.id IS NULL;
