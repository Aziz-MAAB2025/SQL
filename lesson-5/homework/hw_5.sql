...
--- 3. Use UNION to combine results from two queries that select ProductName from Products 
...
--- 8. Use UNION to combine results from two queries 
--- that select ProductName from Products and ProductName from Products_Discounted tables.
...
--- 10. Create a conditional column using IIF that shows 'Expensive' if the 
--- Price is greater than 1000, and 'Affordable' if less, from Products table.
...
--- 13. Write a query that uses CASE to assign 'Top Tier' if SaleAmount > 500, 'Mid Tier' 
...
--- 15. Write a query that uses a CASE statement to determine the discount percentage based on the quantity purchased. Use orders table. Result set should show customerid, quantity and discount percentage. 
