--create database lesson_13_hw_13
--use lesson_13_hw_13
...
select CONCAT_WS (' ', EMPLOYEE_ID, FIRST_NAME, LAST_NAME) as resultcolumn
from Employees
...
---3. That displays the first name and the length of the first name for all employees 
...
SELECT 
    FullName,
    TRIM(PARSENAME(REPLACE(FullName, ' ', '.'), 3)) AS FirstName,
    TRIM(PARSENAME(REPLACE(FullName, ' ', '.'), 2)) AS MiddleName,
    TRIM(PARSENAME(REPLACE(FullName, ' ', '.'), 1)) AS LastName
FROM Students
...
SELECT 
    SequenceNumber,
    STRING_AGG(String, ', ') AS ConcatenatedValues
FROM DMLTable
...
SELECT * FROM Employees
WHERE DATEDIFF(YEAR, HIRE_DATE, '2025-06-12') > 3

---14. Write an SQL statement that determines the most and 
--- least experienced Spaceman ID by their job description.(Personal)

--- Solution:
select JobDescription from Personal
select * from Personal
select SpacemanID from Personal


select JobDescription, min(MissionCount) as min_exper
from Personal
group by JobDescription
select JobDescription, max(MissionCount) as max_exper
from Personal
group by JobDescription

select * from Personal as P LEFT JOIN (
										select JobDescription, MIN(MissionCount) as min_exper
										from Personal
										group by JobDescription
										) as min_exp
										on P.JobDescription = min_exp.JobDescription and p.MissionCount = min_exp.min_exper
										LEFT JOIN (
													select JobDescription, max(MissionCount) as max_exper
													from Personal
													group by JobDescription
													) as max_exp
													on P.JobDescription = max_exp.JobDescription and p.MissionCount = max_exp.max_exper
													WHERE  min_exp.JobDescription IS NOT NULL OR max_exp.JobDescription IS NOT NULL
...
--- Solution:
--- 'tf56sd#%OqH'
DECLARE @INPUT VARCHAR(50) = '.f56sd#%Oq9H',
		@NUMBERS VARCHAR(50) = '',
		@UPPER_CASES VARCHAR(50) = '',
...
WHILE LEN(@INPUT) > 0
BEGIN
	SELECT LEFT(@INPUT,1) AS EACH_CHARACTER
	SET @INPUT = RIGHT(@INPUT, LEN(@INPUT) - 1)
END
--SELECT CASE WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 48 AND 57 THEN 'NUMBERS'
--			WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 65 AND 90 THEN 'UPPER_CASES'
--			WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 97 AND 122 THEN 'LOWER_CASES'
--			ELSE 'SYMBOLS'
--END
...
WHILE LEN(@INPUT) > 0
BEGIN
	--SELECT LEFT(@INPUT, 1),CASE WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 48 AND 57 THEN 'NUMBERS'
	--			WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 65 AND 90 THEN 'UPPER_CASES'
	--			WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 97 AND 122 THEN 'LOWER_CASES'
	--			ELSE 'SYMBOLS'
	--			END
	
	SET @NUMBERS = @NUMBERS + (SELECT CASE WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 48 AND 57 THEN LEFT(@INPUT,1) ELSE '' END)
	SET @SYMBOLS = @SYMBOLS + (SELECT CASE WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 48 AND 57 THEN ''
				WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 65 AND 90 THEN ''
				WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 97 AND 122 THEN ''
				ELSE LEFT(@INPUT,1)
				END)
	SET @UPPER_CASES = @UPPER_CASES + (SELECT CASE WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 65 AND 90 THEN LEFT(@INPUT,1) ELSE '' END)
	SET @LOWER_CASES = @LOWER_CASES + (SELECT CASE WHEN ASCII(LEFT(@INPUT, 1)) BETWEEN 97 AND 122 THEN LEFT(@INPUT,1) ELSE '' END)

	SET @INPUT = RIGHT(@INPUT, LEN(@INPUT)-1)
END

SELECT @NUMBERS AS NUMBERS, @SYMBOLS AS SYMBOLS, @UPPER_CASES AS UPPER_CASES, @LOWER_CASES AS LOWER_CASES

---16. Write an SQL query that replaces each row with the sum of its value and the previous rows' value. (Students table)

--- Solution:
SELECT S1.StudentID,S1.FullName, ISNULL(S1.Grade,0) + ISNULL(S2.Grade,0) AS SUM_SCORE FROM Students AS S1 LEFT JOIN Students AS S2 ON S1.StudentID = S2.StudentID + 1
...
--- Solution:
select * from Student
SELECT S1.StudentID,S1.FullName, ISNULL(S1.Grade,0) + ISNULL(S2.Grade,0) AS SUM_SCORE 
FROM Students AS S1 LEFT JOIN Students AS S2 ON S1.StudentID = S2.StudentID + 1



---19. You have a table with two players (Player A and Player B) and their scores. If a pair of players have multiple entries, aggregate their scores into a single row for each unique pair of players. 
---Write an SQL query to calculate the total score for each unique player pair(PlayerScores)

--- Solution:
SELECT * FROM PlayerScores

SELECT PlayerA, PlayerB, SUM(SCORE) FROM PlayerScores
...
GROUP BY GREATEST(PLAYERA,PLAYERB), 
...
;WITH CTE AS (
				select user1, user2 from FRIENDS as F1
				union all
				select user2, user1 from FRIENDS as F2
				)
				SELECT USER1, COUNT(USER2) AS FRND_CNT FROM CTE
				GROUP BY USER1
				ORDER BY FRND_CNT DESC, USER1 DESC


				;WITH CTE AS (
								SELECT CAST('2025-01-01' AS DATE) AS MY_DATEE
								UNION ALL
								SELECT DATEADD(DAY, 1, MY_DATEE) FROM CTE WHERE MY_DATEE < '2025-12-31'
							)
							SELECT * FROM CTE
